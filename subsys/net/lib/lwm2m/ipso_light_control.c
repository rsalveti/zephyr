...
/*
 * Source material for IPSO Temperature Sensor object (3311):
...
static void *on_time_read_cb(u16_t obj_inst_id, size_t *data_len)
{
	...
	for (i = 0; i < MAX_INSTANCE_COUNT; i++) {
		if (inst[i].obj && inst[i].obj_inst_id == obj_inst_id) {
			if (on_off_value[i]) {
				on_time_value[i] = (k_uptime_get() / 1000) -
					on_time_offset[i];
			}
...
static void *on_time_pre_write_cb(u16_t obj_inst_id, size_t *data_len)
{
	int i;
...
static struct lwm2m_engine_obj_inst *light_control_create(u16_t obj_inst_id)
{
	...
	/* Set default values */
	on_off_value[index] = false;
	dimmer_value[index] = 0;
	on_time_value[index] = 0;
	on_time_offset[index] = 0;
	cumulative_active_value[index].val1 = 0;
	cumulative_active_value[index].val2 = 0;
	power_factor_value[index].val1 = 0;
	power_factor_value[index].val2 = 0;
	colour[index][0] = '\0';
	units[index][0] = '\0';

	/* initialize instance resource data */
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_ON_OFF_ID,
		&on_off_value[index], sizeof(*on_off_value));
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_DIMMER_ID,
		&dimmer_value[index], sizeof(*dimmer_value));
	INIT_OBJ_RES(res[index], i, LIGHT_ON_TIME_ID, 0, NULL,
		sizeof(*on_time_value), on_time_read_cb,
		on_time_pre_write_cb, on_time_post_write_cb, NULL);
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_CUMULATIVE_ACTIVE_POWER_ID,
		&cumulative_active_value[index],
		sizeof(*cumulative_active_value));
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_POWER_FACTOR_ID,
		&power_factor_value[index], sizeof(*power_factor_value));
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_COLOUR_ID,
		colour[index], LIGHT_STRING_SHORT);
	INIT_OBJ_RES_DATA(res[index], i, LIGHT_SENSOR_UNITS_ID,
		units[index], LIGHT_STRING_SHORT);

	inst[index].resources = res[index];
	inst[index].resource_count = i;
	SYS_LOG_DBG("Create IPSO Light Control instance: %d",
		    obj_inst_id);
	return &inst[index];
}
...
static int ipso_light_control_init(struct device *dev)
{
	...
	engine_register_obj(&light_control);
