...
struct spis_nrf5_config {
	...
	uint8_t sck_pin;			/* SCK GPIO pin number */
	uint8_t mosi_pin;			/* MOSI GPIO pin number */
	uint8_t miso_pin;			/* MISO GPIO pin number */
	uint8_t csn_pin;			/* CSN GPIO pin number */
	uint8_t def;				/* Default character */
};
...
static void spis_nrf5_print_cfg_registers(struct device *dev)
{
	...
	__unused uint32_t sck, miso, mosi, csn,
...
#if defined(CONFIG_SOC_SERIES_NRF51X)
		spi_regs->TXDPTR = (uint32_t) tx_buf;
		spi_regs->RXDPTR = (uint32_t) rx_buf;
		spi_regs->MAXTX = tx_buf_len;
		spi_regs->MAXRX = rx_buf_len;
#elif defined(CONFIG_SOC_SERIES_NRF52X)
		spi_regs->TXD.PTR = (uint32_t) tx_buf;
		spi_regs->RXD.PTR = (uint32_t) rx_buf;
...
static void spis_nrf5_complete(struct device *dev, uint32_t error)
{
	...
	__unused uint32_t txd_amount, rxd_amount;
	struct spis_nrf5_data *priv_data = DEV_DATA(dev);
