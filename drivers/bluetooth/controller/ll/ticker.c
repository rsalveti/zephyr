...
#define TICKER_RTC_CC_OFFSET_MIN   (3)	/* refer SUS document */
 ...
 * Global instances
 ****************************************************************************/
static struct ticker_instance gs_instance[2];
...
{
	static struct work s_instance0_worker_irq = {
		0, 0, 0, TICKER_WORKER0_IRQn, (work_fp) ticker_worker,
		&gs_instance[0]
	};
	 ...
	 * schedule.
	 */
	(void)work_schedule(&s_instance0_worker_irq, chain);
}

static void ticker_instance0_job_sched(uint8_t chain)
{
	static struct work s_instance0_job_irq = {
		0, 0, 0, TICKER_JOB0_IRQn, (work_fp) ticker_job,
		&gs_instance[0]
	};
	 ...
	 * schedule.
	 */
	(void)work_schedule(&s_instance0_job_irq, chain);
}
...
{
	static struct work s_instance1_worker_irq = {
		0, 0, 0, TICKER_WORKER1_IRQn, (work_fp) ticker_worker,
		&gs_instance[1]
	};
	 ...
	 * schedule.
	 */
	(void)work_schedule(&s_instance1_worker_irq, chain);
}

static void ticker_instance1_job_sched(uint8_t chain)
{
	static struct work s_instance1_job_irq = {
		0, 0, 0, TICKER_JOB1_IRQn, (work_fp) ticker_job,
		&gs_instance[1]
	};
	 ...
	 * schedule.
	 */
	(void)work_schedule(&s_instance1_job_irq, chain);
}
...
uint32_t ticker_init(uint8_t instance_index, uint8_t count_node, void *node,
		    ...
		    void *user_op)
{
	struct ticker_instance *instance = &gs_instance[instance_index];
	struct ticker_user *users;
...
#if NRF51 || NRF52
	NVIC_SetPriority(TICKER_WORKER0_IRQn, TICKER_WORKER0_PRIORITY);
	NVIC_EnableIRQ(TICKER_WORKER0_IRQn);
	NVIC_SetPriority(TICKER_JOB0_IRQn, TICKER_JOB0_PRIORITY);
	NVIC_EnableIRQ(TICKER_JOB0_IRQn);
	NVIC_SetPriority(TICKER_WORKER1_IRQn, TICKER_WORKER1_PRIORITY);
	NVIC_EnableIRQ(TICKER_WORKER1_IRQn);
	NVIC_SetPriority(TICKER_JOB1_IRQn, TICKER_JOB1_PRIORITY);
	NVIC_EnableIRQ(TICKER_JOB1_IRQn);
#endif
...
{
	if (gs_instance[instance_index].fp_worker_sched) {
		gs_instance[instance_index].fp_worker_sched(1);
	}
}
