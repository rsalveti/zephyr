...
#if (TICKER_WORKER0_PRIORITY == TICKER_JOB0_PRIORITY)
static void ticker_job_disable(uint32_t status, void *op_context)
{
	...
	if (_radio.state != STATE_NONE) {
		NVIC_DisableIRQ(TICKER_JOB0_IRQn);
	}
}
#endif
				   ...
				   uint16_t lazy, void *context)
{
	static struct work s_work_radio_active = { 0, 0, 0,
		TICKER_WORKER0_IRQn, (work_fp) work_radio_active, (void *)1 };
	uint32_t retval;
...
static void event_xtal(uint32_t ticks_at_expire, uint32_t remainder,
				 uint16_t lazy, void *context)
{
	static struct work s_work_xtal_start = { 0, 0, 0, TICKER_WORKER0_IRQn,
	...
	if (retain) {
		if (!s_xtal_retained) {
			static struct work s_work_xtal_start = { 0, 0, 0,
				TICKER_WORKER0_IRQn, (work_fp) work_xtal_start,
				0 };
			uint32_t retval;
		...
		if (s_xtal_retained) {
			static struct work s_work_xtal_stop = { 0, 0, 0,
				TICKER_WORKER0_IRQn, (work_fp) work_xtal_stop,
				0 };
			uint32_t retval;
...
static void event_stop(uint32_t ticks_at_expire, uint32_t remainder,
				 uint16_t lazy, void *context)
{
	static struct work s_work_radio_stop = {
		0, 0, 0, TICKER_WORKER0_IRQn, (work_fp) work_radio_stop, 0 };
	uint32_t retval;
...
static void adv_obs_conn_configure(uint8_t phy)
{
	...
	NVIC_SetPriority(RADIO_IRQn, TICKER_WORKER0_PRIORITY);
			...
			uint16_t event_counter, uint32_t ticks_at_expire)
{
	...
	if (conn->llcp.connection_update.state) {
		 ...
		 * to avoid RTC being stopped.
		 */
		irq_disabled = NVIC->ISER[0] &
			(1 << ((uint32_t)(TICKER_JOB0_IRQn) & 0x1F));
		NVIC_DisableIRQ(TICKER_JOB0_IRQn);
...
static void connection_release(struct connection *conn)
{
	...
	/* Enable Timer Job, we are in a radio event which disabled it if
					...
					uint32_t ticks_active_to_start)
{
	...
	static struct work s_work_radio_deassert = { 0, 0, 0,
		TICKER_WORKER0_IRQn, (work_fp) work_radio_deassert, 0 };
	...
	if (ticker_status_event == TICKER_STATUS_BUSY) {
		...
		while (ticker_status_event == TICKER_STATUS_BUSY) {
			__WFE();
			__SEV();
			__WFE();
		}
	...
	if (ticker_status_event == TICKER_STATUS_SUCCESS) {
		...
		if (ticker_status_pre_event == TICKER_STATUS_BUSY) {
			...
			/** @todo design to avoid this wait */
			while (ticker_status_pre_event == TICKER_STATUS_BUSY) {
				__WFE();
				__SEV();
				__WFE();
			}
		...
		if (ticker_status_stop == TICKER_STATUS_BUSY) {
			...
			/** @todo design to avoid this wait */
			while (ticker_status_stop == TICKER_STATUS_BUSY) {
				__WFE();
				__SEV();
				__WFE();
			}
		...
		if (_radio.role != ROLE_NONE) {
			static struct work s_work_radio_stop = { 0, 0, 0,
				TICKER_WORKER0_IRQn, (work_fp) work_radio_stop,
				0 };
			uint32_t retval;
